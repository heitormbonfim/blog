...
test("login with credentials", async () => {
  const response = await fetch(url + "/login", {
    ...
    body: JSON.stringify({ email: "heitormbonfim@email", password: "saadasdasdsa" }),
  });
...
test("login with invalid token", async () => {
  const response = await fetch(url + "/login", {
    method: "GET",
    headers: {
      auth: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6 BREAKING THE TOKEN ImhlaXRvcm1ib25maW1AZW1haWwuY29tIiwibmFtZSI6eyJmaXJzdCI6IkhlaXRvciIsImxhc3QiOiJCb25maW0ifSwicm9sZSI6InVzZXIiLCJpYXQiOjE3MTM0NjY5ODJ9.69Qu0xv8qWKiwioqR4kpGa5PfjiWAiqhpkLTmMePFg8",
    },
  });
